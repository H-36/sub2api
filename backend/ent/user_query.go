...
type UserQuery struct {
	...
	withRedeemCodeClaims      *RedeemCodeClaimQuery
...
// QueryRedeemCodeClaims chains the current query on the "redeem_code_claims" edge.
func (_q *UserQuery) QueryRedeemCodeClaims() *RedeemCodeClaimQuery {
	query := (&RedeemCodeClaimClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRedeemCodeClaims:      _q.withRedeemCodeClaims.Clone(),
...
// WithRedeemCodeClaims tells the query-builder to eager-load the nodes that are connected to
// the "redeem_code_claims" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithRedeemCodeClaims(opts ...func(*RedeemCodeClaimQuery)) *UserQuery {
	query := (&RedeemCodeClaimClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRedeemCodeClaims = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withRedeemCodeClaims; query != nil {
		if err := _q.loadRedeemCodeClaims(ctx, query, nodes,
			func(n *User) { n.Edges.RedeemCodeClaims = []*RedeemCodeClaim{} },
			func(n *User, e *RedeemCodeClaim) { n.Edges.RedeemCodeClaims = append(n.Edges.RedeemCodeClaims, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadRedeemCodeClaims(ctx context.Context, query *RedeemCodeClaimQuery, nodes []*User, init func(*User), assign func(*User, *RedeemCodeClaim)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(redeemcodeclaim.FieldUserID)
	}
	query.Where(predicate.RedeemCodeClaim(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RedeemCodeClaimsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
