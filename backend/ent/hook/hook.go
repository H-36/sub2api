...
// The RedeemCodeClaimFunc type is an adapter to allow the use of ordinary
// function as RedeemCodeClaim mutator.
type RedeemCodeClaimFunc func(context.Context, *ent.RedeemCodeClaimMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RedeemCodeClaimFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RedeemCodeClaimMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RedeemCodeClaimMutation", m)
}
