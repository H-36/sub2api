...
func TestListPlazaGroups_DeepSeekBrandUsesOpenAIExecutionPlatform(t *testing.T) {
	channels := []Channel{
		plazaPricedChannel(1, "deepseek", []int64{10}, PlatformOpenAI, DeepSeekV4FlashModel),
	}
	groups := []Group{{ID: 10, Name: "deepseek", Platform: PlatformDeepSeek, RateMultiplier: 1}}
	...
	require.Len(t, out, 1)
	require.Len(t, out[0].Models, 1)
	require.Equal(t, DeepSeekV4FlashModel, out[0].Models[0].Name)
	require.Equal(t, PlatformDeepSeek, out[0].Models[0].Platform)
}
