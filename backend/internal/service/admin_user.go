...
func (s *adminServiceImpl) GetRedeemCodeClaims(ctx context.Context, id int64) ([]RedeemCodeClaim, error) {
	...
	if code == nil || code.Type != RedeemTypeWelfare {
...
func (s *adminServiceImpl) GenerateRedeemCodes(ctx context.Context, input *GenerateRedeemCodesInput) ([]RedeemCode, error) {
	...
	if customCode != "" && input.Count != 1 && input.Type != RedeemTypeWelfare {
		return nil, errors.New("count must be 1 when custom code is provided")
	}
	if input.Type == RedeemTypeWelfare {
		if customCode == "" {
			return nil, errors.New("custom code is required for welfare redeem code")
		}
		if input.Count <= 0 {
			return nil, errors.New("count must be greater than zero for welfare redeem code")
		}
		code := RedeemCode{
			Code:         customCode,
			Type:         input.Type,
	...
	for i := 0; i < input.Count; i++ {
		...
		if codeValue == "" {
			generated, err := GenerateRedeemCode()
			if err != nil {
				return nil, err
			}
			codeValue = generated
...
func (s *adminServiceImpl) DeleteRedeemCode(ctx context.Context, id int64) error {
	...
	// 福利码可被多人领取，不能仅依赖 status 判断；只要产生过领取记录就必须保留，
	// 否则会破坏兑换审计与用户领取历史。普通已使用兑换码同样禁止删除。
